...
__global__ __launch_bounds__(kNumTMAThreads + kNumMathThreads, 1) void
swapAB_sm90_fp8_gemm_1d2d_impl(float* sfb, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_a,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_b,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_d,
    ...
    // Scaling checks
    DG_STATIC_ASSERT(BLOCK_K == 128, "Only support per-128-channel FP8 scaling");
    DG_STATIC_ASSERT(BLOCK_N == 64 or BLOCK_N == 128 or BLOCK_N == 256  , "Only support BLOCK_N=64 or 128 or 256");

    // Types
    using WGMMA = typename FP8MMASelector<BLOCK_M>::type;
    using Barrier = cutlass::arch::ClusterTransactionBarrier;
    DG_STATIC_ASSERT(BLOCK_M == WGMMA::N, "Invalid block size");
    ...
    constexpr uint32_t n_nums =  (BLOCK_N+127)/128;
    // Shared memory
    static constexpr uint32_t SMEM_D_SIZE = BLOCK_M * BLOCK_N * sizeof(__nv_bfloat16);
    static constexpr uint32_t SMEM_A_SIZE_PER_STAGE = BLOCK_M * BLOCK_K * sizeof(__nv_fp8_e4m3);
    static constexpr uint32_t SMEM_B_SIZE_PER_STAGE = BLOCK_N * BLOCK_K * sizeof(__nv_fp8_e4m3);
    static constexpr uint32_t SMEM_SFA_SIZE_PER_STAGE = BLOCK_M * sizeof(float);
    const uint32_t& shape_k_scales = ceil_div(shape_k, BLOCK_K);
    const uint32_t& smem_sfb_size = align<uint32_t>(shape_k_scales * n_nums * sizeof(float), sizeof(Barrier));
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            // Decide the number of scales B to load
            DG_TRAP_ONLY_DEVICE_ASSERT(shape_n % 8 == 0);
            uint32_t num_sfb = shape_k_scales * n_nums;
            ...
            // Accumulation for WGMMA or CUDA promotion
            float accum[WGMMA::kNumAccum], final_accum[WGMMA::kNumAccum * n_nums] = {0};
            ...
            launch_k_iterations([&](uint32_t k_iter, auto divisible_type, auto skip_type, auto _) {
                ...
                for (uint32_t s = 0; s < kNumInnerStages; ++ s) {                  
                    ...
                    #pragma unroll
                    for (uint32_t local_idx = 0; local_idx < n_nums; ++ local_idx) {
                      	auto n_offset = local_idx * 128;
                        // Read B scales
                        float scale_b = ld_shared(smem_sfb + local_idx * shape_k_scales + k_iter * kNumStages + s);  
                        ...
                        for (uint32_t k = 0; k < BLOCK_K / WGMMA::K; ++ k) {
                            ...
                            auto desc_b = make_smem_desc(smem_b[s] + (math_wg_idx * WGMMA::M + n_offset) * BLOCK_K + k * WGMMA::K, 1);
                            WGMMA::wgmma(desc_b, desc_a, accum, k);
                        }
                        warpgroup_commit_batch();
                        #pragma unroll
                        for (uint32_t i = 0; i < WGMMA::kNumAccum; ++ i)
                        ...
                        #pragma unroll
                        for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                            int in_blk_c   = (lane_idx & 3) * 2; 
                            int col_base   =  (i << 3) + in_blk_c;  
                            float2 sa = ld_shared(reinterpret_cast<const float2*>(smem_sfa[s] + col_base));
                            float scale_0_0 = sa.x * scale_b, scale_1_0 = sa.y * scale_b;
                            shifted_accum[i * 4 + 0] += (scale_0_0) * accum[i * 4 + 0];
                            shifted_accum[i * 4 + 1] += (scale_1_0) * accum[i * 4 + 1];
                            shifted_accum[i * 4 + 2] += (scale_0_0) * accum[i * 4 + 2];
                            shifted_accum[i * 4 + 3] += (scale_1_0) * accum[i * 4 + 3];
                        }
                    }
                }
            ...
            }, not scheduler.is_computation_valid(m_block_idx, 0));
            ...
            for (uint32_t local_idx = 0; local_idx < n_nums; ++ local_idx) {
                ...
                #pragma unroll
                for (auto i = 0; i < WGMMA::kNumAccum / 4; ++ i) { // WGMMA::M * WGMMA::N / 128 / 4
                    // Swizzle or padding into the correct address
                    uint8_t* smem_ptr = nullptr;
                    if constexpr (kSwizzleDMode > 0) {
                        uint32_t src_row  = warp_idx * WGMMA_M_PER_WARP + lane_idx;
                        uint32_t src_col8 = i * 8;              
                        uint32_t blk_r = src_row  >> 3;          
                        uint32_t blk_c = src_col8 >> 3;        
                        uint32_t in_r  = src_row  &  7;          
                        uint32_t in_c  = src_col8 &  7;          
                        uint32_t dst_row   = blk_c * 8 + in_r; 
                        uint32_t dst_col8  = blk_r * 8 + in_c;   
                        dst_col8 += n_offset; 
                        uint32_t warp_idx_tr = dst_row / WGMMA_M_PER_WARP;
                        uint32_t lane_idx_tr = dst_row % WGMMA_M_PER_WARP;
                        constexpr uint32_t kNumBankGroupBytes = 16;
                        uint32_t i_tr          = dst_col8 >> 3;              
                        uint32_t atom_offset    = i_tr / (TMA_D_BLOCK_N / 8);
                        uint32_t in_atom_offset = i_tr % (TMA_D_BLOCK_N / 8);
                        uint32_t bank_group_idx =
                            in_atom_offset + lane_idx_tr * (kSwizzleDMode / kNumBankGroupBytes);
                        constexpr bool kHasShortcut = (kSwizzleDMode / kNumBankGroupBytes) == 8;
                        uint32_t row = kHasShortcut ? (in_atom_offset / 8 + lane_idx_tr)
                                                    : (bank_group_idx / 8);
                        uint32_t col = kHasShortcut ? (in_atom_offset)
                                                    : (bank_group_idx % 8);
                        col ^= row % (kSwizzleDMode / 16);
                        smem_ptr = reinterpret_cast<uint8_t*>(smem_d) +
                                warp_idx_tr * (WGMMA_M_PER_WARP * kSwizzleDMode) + 
                                atom_offset * BLOCK_M * kSwizzleDMode +     
                                row *  (kNumBankGroupBytes * 8) +                  
                                col * kNumBankGroupBytes;                         
                    } else { 
                        // No swizzling, just padding
                        int row   = warp_idx * WGMMA_M_PER_WARP + lane_idx; 
                        int col8  = i * 8;       
                        ...
                        int dst_blk_r = blk_c;
                        int dst_blk_c = blk_r;
                        int dst_row   = dst_blk_r * 8 + in_r;   
                        int dst_col8  = dst_blk_c * 8 + in_c;  
                        
                        smem_ptr = reinterpret_cast<uint8_t*>(
                            smem_d + n_offset +  dst_row * BLOCK_N + dst_col8);
                    }
