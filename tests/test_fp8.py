...
def test_m_grouped_gemm_masked() -> None:
    ...
    for kernel_type, enable_overlap, num_groups, max_m, expected_m_per_group, n, k in enumerate_m_grouped_masked(torch.float8_e4m3fn):
        ...
        # ==========================
        # Correctness tests (保持原逻辑)
        # ==========================
        for i in range(10):
            a, b, masked_m, d, ref_d, signal = generate_m_grouped_masked(
                num_groups, max_m, expected_m_per_group, n, k,
                use_ue8m0=use_ue8m0, enable_overlap=enable_overlap
            )
            ...
            for j in range(num_groups):
                ...
                assert diff < 0.001, (
                    f'{max_m=}, {n=}, {k=}, {j=}, masked_m={masked_m[j]}, '
                    f'{kernel_opt}, {num_groups=}, {diff:.5f}'
                )
        ...
        # ============ Torch Profiler ============
        trace_file = f"traces/mgroup_masked_{case_idx}_groups{num_groups}_n{n}_k{k}_overlap{enable_overlap}.json"
        ...
        with profiler.profile(
            ...
            torch.cuda.nvtx.range_push(f"mgroup_masked_case_{case_idx}")
            test_func()
            torch.cuda.nvtx.range_pop()

        prof.export_chrome_trace(trace_file)
        print(f"[Profiler] Chrome trace saved to {trace_file}")
